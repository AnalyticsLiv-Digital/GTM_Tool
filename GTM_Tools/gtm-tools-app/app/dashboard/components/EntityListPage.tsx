...
export type EntityListPageProps<T> = {
  ...
  // ✅ NEW: export callback (clean approach)
  ...
  // ✅ Optional filter UI like dropdowns
  filters?: ReactNode;
...
export function EntityListPage<T>({
  ...
  return (
    <div className="relative">
      ...
      <div className="sticky top-0 z-40 bg-page/95 backdrop-blur-md border-b border-line">
        <div className="px-1 pt-1 pb-4">
          ...
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-4 mb-6">
            ...
            <div className="flex gap-2">
              ...
              >
                ...
                Clone{selectedIds.length > 0 ? ` (${selectedIds.length})` : ""}
              </button>
              ...
              <button
                ...
                disabled={selectedIds.length === 0}
                className="px-4 py-2 rounded-xl border border-red-500/20 bg-red-500/10 text-red-500 hover:bg-red-500/15 transition disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
              >
                <Trash2 size={16} />
          ...
          {!workspaceSelected && (
            <div className="mt-4 bg-(--warn)/10 border border-(--warn)/25 text-(--warn) px-4 py-3 rounded-lg text-[13px]">
  ...
  `}
                      >
                        ...
                        {columns.map((c) => (
                          <td
                            key={c.label}
                            className="px-4 py-3 text-fg align-top"
                          >
                            {c.render(item)}
                          </td>
                        ))}
        ...
        <div className="mt-4 font-mono text-[11px] text-faint">
          Showing {filtered.length} {singularName}(s) · selected{" "}
          {selectedIds.length}
        </div>
      </div>
    </div>
