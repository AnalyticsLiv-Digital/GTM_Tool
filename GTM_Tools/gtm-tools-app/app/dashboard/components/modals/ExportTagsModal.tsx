...
export default function ExportTagsModal({
  ...
  async function handleExportTags() {
    ...
    try {
      ...
      // ------------------------------------------
      // Fetch ALL source triggers + variables once
      // ------------------------------------------
      ...
      // ------------------------------------------
      // Fetch DESTINATION triggers + variables once
      // ------------------------------------------
      ...
      const destTriggers = destTriggersData.trigger || [];
      ...
      const destVariables = destVariablesData.variable || [];

      // ------------------------------------------
      // Trigger mapping (sourceTriggerId -> destTriggerId)
      // ------------------------------------------
      const triggerIdMap: Record<string, string> = {};

      const destTriggerNameMap: Record<string, any> = {};
      destTriggers.forEach((t: any) => {
      ...
      const destVariableNameSet = new Set<string>(
        destVariables.map((v: any) => v.name)
      );

      // ------------------------------------------
      // Template cache map (sourceTemplateId -> destTemplateId)
      // ------------------------------------------
      const templateIdMap: Record<string, string> = {};

      // ------------------------------------------
      // Export triggers used by tags
      // ------------------------------------------
      ...
      // ------------------------------------------
      // Export templates ONLY ONCE per templateId
      // ------------------------------------------
      async function exportTemplateIfNeeded(tag: any): Promise<string> {
        ...
        if (!fetchTemplateRes.ok) {
          throw new Error(fetchTemplateData?.error || "Failed to fetch template");
        }
        ...
        while (attempt < maxRetries) {
          ...
          const createTemplateData = await createTemplateRes.json().catch(() => ({}));
          ...
          // If template already exists, don't fail all tags
          const msg =
          ...
          if (msg.toLowerCase().includes("already exists")) {
            // if duplicate, just skip mapping and use original type
            return tag.type;
          }
      ...
      for (const tag of selectedTags) {
        try {
          ...
          if (!exportSuccess) throw new Error("Tag export failed after retries");
          ...
          <div className="col-span-5 border-r bg-gray-50 p-4">
            ...
            <div className="bg-white border rounded-xl p-3 mb-4">
              ...
              <div className="flex flex-wrap gap-2">
                {Object.keys(tagTypeSummary).map((type) => (
                  <span
                    key={type}
                    className="px-2 py-1 rounded-full text-xs border bg-gray-100"
                  >
                    {type}: {tagTypeSummary[type]}
                  </span>
                ))}
              </div>
            </div>
...
//       description: "Required triggers, variables, and templates will be created in the destination workspace if they don't already exist there. Existing items are not modified.",
//       confirmLabel: "Export",
//     });
//     if (!ok) return;
...
//         const confirmVar = await confirmDialog({
//           title: `Export ${missingVariables.length} missing variable(s)?`,
//           description: "These variables are referenced by the tags you're exporting but don't exist in the destination workspace yet.",
//           confirmLabel: "Export variables",
//         });
